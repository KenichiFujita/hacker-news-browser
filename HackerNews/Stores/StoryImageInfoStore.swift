...
class StoryImageInfoStore {
    ...
    func imageIconURL(url: String, host: String, completionHandler: @escaping (URL?) -> Void) {
        queue.async {
            if let cachedStoryImageInfo = self.storyImageInfoCache.imageInfos[host] {
                ...
                self.imageInfo(url: url) { (storyImageInfo) in
                    ...
                    self.storyImageInfoCache.addStoryImageInfo(storyHost: host, storyImageInfo: storyImageInfo)
                    DispatchQueue.main.async {
